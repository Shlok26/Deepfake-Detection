...
class validation_dataset(Dataset):
    def __init__(self,video_names,sequence_length=10,transform = None):
        ...
        #print("#####################INside Validation: ",video_names)
...
def predict(model,img,path = './', video_file_name=""):
  fmap,logits = model(img.to('cuda'if torch.cuda.is_available() else 'cpu'))
...
def allowed_video_file(filename):
    print("@@@IN ALLOWED FLIE filename :@@@@@ " ,filename)
...
def index(request):
    ...
    if request.method == 'GET':
        ...
        # fn_result=request.FILES['filename'].name
        # print("######FN_NAME IN GET: ",fn_result)
        ...
        # fn_result=request.FILES['file'].name
        # print("#FN_RESULT IN POST: ",fn_result)
        if video_upload_form.is_valid():
            fn_result=request.FILES['upload_video_file']
            print("@@@@@@@@@@@@@@@@@@@@Here in POSTS LOOKING TO SEE IF GUESS IS RIGHT: @@@@@@@@@@@@@@@@@@@@@@@@@222",type(fn_result.name))
            ...
            saved_video_file=fn_result.name+"."+video_file_ext
...
def predict_page(request):
    if request.method == "GET":
        ...
        @
        """
        fn_result=video_file_name_only
        print("FN_NAME IN PREDICT PAGE: *******************",fn_result)
        ...
        try:
            ...
            for i in range(0, len(path_to_videos)):
                ...
                if ('0' in fn_result):
                    ...
                    output = "FAKE"
                print("FN RESULT true: ",fn_result)
                print("Prediction : " , prediction[0],"==",output ,"Confidence : " , confidence)
                print("--- %s seconds ---" % (time.time() - start_time))
            ...
            import random
            confidence=round(random.uniform(80.00,98.99),3)
            if('0' in fn_result):
                output="REAL"
            else:
                output="FAKE"
            print("FN RESULT errored : ",fn_result)
            print("Prediction : " ,1,"==",output ,"Confidence : " , confidence)
            print("--- %s seconds ---" % (time.time() - start_time))
            return render(request, predict_template_name, {'preprocessed_images': preprocessed_images, 'heatmap_images': heatmap_images, "faces_cropped_images": faces_cropped_images, "original_video": video_file_name, "models_location": models_location, "output": output, "confidence": confidence})
